...
public class Derslik extends JInternalFrame {
	...
	public Derslik() {
		...
		//textFields
				...
				//Gözetmen Adý ComboBox Doldurulmasý		
				try {		
					Connection con = DBConnection.dbConnector();		
	...
	public static void TabloYazdir() {
		...
		try {
		    ...
		    Statement state = con.createStatement();
			ResultSet rs = state.executeQuery("SELECT Derslik.*, Gozetmen.Adi AS \"Gözetmen Adý\"\r\n" + 
												"FROM Derslik INNER JOIN\r\n" + 
												"Gozetmen ON Derslik.GozetmenID = Gozetmen.ID");			//istenilen query 'nin yazýmýnýn geçcekleþtirileceði alan
